defmodule BlogAppWeb.PostControllerTest do
  ...
  describe "unauthenticated" do
    ...
    test "creating post requires authentication", %{conn: conn} do
      _post = fixture(:post)
      user = fixture(:user)
      conn = post conn, Routes.user_post_path(conn, :create, user), post: %{}
  ...
  describe "authenticated" do
    ...
    test "creates post and redirects to index when data is valid", %{conn: conn, user: user} do
      ...
      conn = post conn, Routes.user_post_path(conn, :create, user), post: post, categories: categories
    ...
    test "does not create post and renders errors when data is invalid", %{conn: conn, user: user} do
      categories = %{@category.name => ""}
      conn = post conn, Routes.user_post_path(conn, :create, user), post: invalid_attrs(), categories: categories
      assert html_response(conn, 200) =~ "New Post"
    end
    ...
    test "updates chosen post and redirects when data is valid", %{conn: conn, user: user} do
      ...
      categories = %{@category.name => ""}
      conn = put conn, Routes.user_post_path(conn, :update, user, post), post: updated, categories: categories
      assert redirected_to(conn) == Routes.post_path(conn, :show, updated.permalink)
    ...
    test "does not update chosen post and renders errors when data is invalid", %{conn: conn, user: user} do
      ...
      {:ok, post} = Blog.create_post(user, attrs)
      categories = %{@category.name => ""}
      conn = put conn, Routes.user_post_path(conn, :update, post.user_id, post), post: invalid, categories: categories
      assert html_response(conn, 200) =~ "Edit Post"
    end
