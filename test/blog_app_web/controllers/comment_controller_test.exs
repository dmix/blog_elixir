defmodule BlogAppWeb.CommentControllerTest do
  ...
  describe "unauthenticated user" do
    ...
    test "creates and renders comment when data is valid", %{conn: conn} do
      ...
      conn = post conn, Routes.post_comment_path(conn, :create, post), comment: comment 
    ...
    test "updates and renders chosen comment when data is valid", %{conn: conn} do
      ...
      conn = put conn, Routes.post_comment_path(conn, :update, post, comment), comment: valid 
    ...
    test "does not update chosen comment and renders errors when data is invalid", %{conn: conn} do
      ...
      conn = put conn, Routes.post_comment_path(conn, :update, post, comment), comment: invalid 
      assert json_response(conn, 422)["errors"] != %{}
    end

    test "deletes chosen comment", %{conn: conn} do
      %{post: post, comment: comment} = fixture(:comment)
      conn = delete conn, Routes.post_comment_path(conn, :delete, post, comment)
  ...
  describe "authenticated admin" do
    ...
    #   conn = put conn, Routes.post_comment_path(conn, :update, comment), comment: update_attrs()
